...
<div class="container-fluid">
    ...
    <div class="card" style="width:60%;margin: 0 auto;">
        ...
        <div class="card-body">
            <span><b>Ubah Surat Masuk</b></span>
            <hr>
            <form action="{{ route('masuk.update', $get->id) }}" method="post" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="row">
                    <div class="col-md-6">
                ...
                <div class="row">
                    <div class="col-md-4">
                        ...
                        <input type="text" name="disposisi" class="form-control {{ $errors->has('disposisi') ? 'is-invalid' : ''}}" value="{{ $get->disposisi }}" required>
                        {!! $errors->first('disposisi', '<span class="invalid-feedback">:message</span>') !!}
                    </div>
                    <div class="col-md-4">
                        <label for="">Perihal</label>
                        <input type="text" name="perihal_disposisi" class="form-control {{ $errors->has('perihal_disposisi') ? 'is-invalid' : ''}}" value="{{ $get->perihal_disposisi }}" required>
                        {!! $errors->first('perihal_disposisi', '<span class="invalid-feedback">:message</span>') !!}
                    </div>
                    <div class="col-md-4">
                        <label for="">Tujuan Surat</label>
                        <input type="text" name="tujuan" class="form-control {{ $errors->has('tujuan') ? 'is-invalid' : ''}}" value="{{ $get->tujuan }}" required>
